...
Public Class Main_Installer
    ...
    Private Sub Btn_Install_Click(sender As Object, e As EventArgs) Handles Btn_Install.Click
        ...
        If System.IO.File.Exists(FILE_NAME) = True Then
            ...
            objWriter.WriteLine(""" Version=""16.0.14332.20176"" AllowCdnFallback=""FALSE"" ForceUpgrade=""FALSE""> ")
            objWriter.WriteLine("<Product ID=""ProPlus2021Volume""  PIDKEY=""FXYTK-NJJ8C-GB6DW-3DYQT-6F7TH"">")              '//校園授權版
            ...
            '直接不安裝Teams
            objWriter.WriteLine("<ExcludeApp ID=""Teams"" />")

            '直接不安裝OneDrive
            objWriter.WriteLine("<ExcludeApp ID=""OneDrive"" />")
            ...
            If (sel_visio.Checked = True) Then
                objWriter.WriteLine("<Product ID=""VisioPro2021Volume""  PIDKEY=""KNH8D-FGHT4-T8RK3-CTDYJ-K2HT4"">")              '//校園授權版
                objWriter.Write("   <Language ID=""")
                objWriter.WriteLine(language & """ />")
                objWriter.WriteLine("</Product>")
            End If
            ...
            If (sel_project.Checked = True) Then
                objWriter.WriteLine("<Product ID=""ProjectPro2021Volume""  PIDKEY=""FTNWT-C6WBT-8HMGF-K9PRX-QV9H8"">")              '//校園授權版
                objWriter.Write("   <Language ID=""")
                objWriter.WriteLine(language & """ />")
                objWriter.WriteLine("</Product>")
            End If
                ...
                If response = MsgBoxResult.Ok Then
                    If System.IO.File.Exists("bin\Setup.exe") = True Then
                        If System.IO.File.Exists(Path.GetTempPath() + "configuration.xml") = True Then
                            Shell("bin\setup.exe /configure " + Chr(34) + Path.GetTempPath() + "configuration.xml" + Chr(34))     '//用customize方式可避免下一次安裝要新增程式，但安裝時未勾選而被解除安裝的狀況
                            Close() '安裝完成後關閉選擇畫面
                        Else                                                                                                                                                                         '但是必須要偵測2021 是否 installed
    ...
    Private Sub InstalledAppCheck()
        ...
        Dim appFilename() As String = {"WINWORD", "EXCEL", "POWERPNT", "OUTLOOK", "ONENOTE", "MSACCESS", "MSPUB", "LYNC", "WINPROJ", "VISIO"}
        Dim appChkbox() As CheckBox = New CheckBox() {sel_world, sel_excel, sel_powerpoint, sel_outlook, sel_onenote, sel_access, sel_publisher, sel_skype, sel_project, sel_visio}
        ...
        For i = 0 To 1
            For j = 0 To (appFilename.Length) - 1
    ...
    Private Sub sel_access_CheckedChanged(sender As Object, e As EventArgs) Handles sel_world.CheckedChanged, sel_excel.CheckedChanged, sel_powerpoint.CheckedChanged, sel_outlook.CheckedChanged, sel_onenote.CheckedChanged, sel_access.CheckedChanged, sel_publisher.CheckedChanged, sel_skype.CheckedChanged, sel_project.CheckedChanged, sel_visio.CheckedChanged
        ...
        If asked = False Then
            If appExist((sender).tag) = True Then
                asked = True
                If Not (GetSystemDefaultLangID = "1028") Then
                    msgtext = "The application you unchecked has already installed in this computer, this operation will remove it. Continue?"
